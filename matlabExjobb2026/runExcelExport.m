...
CCt_q=zeros(nPeriods,1); CCtr_q=zeros(nPeriods,1); CCtl_q=zeros(nPeriods,1); CCx_q=zeros(nPeriods,1);
...
for p = 1:nPeriods
  ...
  if ~isempty(idx)
    ...
    CCt_q(p)     = sum(dr.dFX_cc_total(idx));
    CCtr_q(p)    = sum(dr.dFX_cc_trans(idx));
    CCtl_q(p)    = sum(dr.dFX_cc_transl(idx));
    CCx_q(p)     = sum(dr.dFX_cc_cross(idx));
...
% PAM CC three-way decomposition: total = trans + transl + cross
CCt_a=zeros(nYears,1); CCtr_a=zeros(nYears,1); CCtl_a=zeros(nYears,1); CCx_a=zeros(nYears,1);
...
for y = 1:nYears
  ...
  CCt_a(y)     = sum(CCt_q(qM));   CCtr_a(y)    = sum(CCtr_q(qM));
  CCtl_a(y)    = sum(CCtl_q(qM));  CCx_a(y)     = sum(CCx_q(qM));
...
fprintf('%-6s %13s %13s %13s %13s %13s %13s %13s\n', ...
  'Year','TI (bonds)','TI (bonds+BOM)','OCI','CC_total','CC_trans','CC_transl','CC_cross');
fprintf('%s\n',repmat('-',1,104));
for y = 1:nYears
  fprintf('%-6d %13.0f %13.0f %13.0f %13.0f %13.0f %13.0f %13.0f\n', ...
    uniqueYears(y),TI_a(y),TI_BOM_a(y),OCI_a(y), ...
    CCt_a(y),CCtr_a(y),CCtl_a(y),CCx_a(y));
end
fprintf('%s\n',repmat('-',1,104));
fprintf('%-6s %13.0f %13.0f %13.0f %13.0f %13.0f %13.0f %13.0f\n','TOTAL', ...
  sum(TI_a),sum(TI_BOM_a),sum(OCI_a), ...
  sum(CCt_a),sum(CCtr_a),sum(CCtl_a),sum(CCx_a));
...
hdrQ = {'Quarter', ...
  ...
  'PAM_CC_trans [SEK]', 'PAM_CC_transl [SEK]', 'PAM_CC_cross [SEK]', ...
...
qData = [qLabels, num2cell([...
  TI_q, TI_BOM_q, OCI_q, CCt_q, CCtr_q, CCtl_q, CCx_q, CCt_LY_q, CCtr_LY_q, CCtl_LY_q, ...
...
hdrPV = [{'Date', ...
  ...
  {'V_SEK_const [SEK] (both rates frozen — prior-year same-month avg)', ...
  'V_SEK_trans_const [SEK] (foreign frozen, EUR/SEK actual)', ...
  'V_SEK_transl_const [SEK] (foreign actual, EUR/SEK frozen)', ...
  'V_SEK_LYRates [SEK] (last-year same-date rates)', ...
  'V_SEK_TranslLY [SEK] (actual trans x last-year EUR/SEK)', ...
  'EUR/SEK_CompRate'}];
pvData = [dateStrs, num2cell([dr.V_EUR, dr.V_SEK, ...
  sum(cashEUR,2), compEUR, wipEUR, bondsEUR, bondsByCcyEUR, ...
  dr.V_SEK_const, dr.V_SEK_trans_const, dr.V_SEK_transl_const, dr.V_SEK_LY, dr.V_SEK_transl_LY, ...
  dr.f_EUR_SEK_comp])];
...
hdrFX = {'Date', ...
  ...
  'CC_total_daily [SEK] (Eq.4.47 — three-way decomposition total)', ...
  'CC_trans_daily [SEK] (pure foreign at frozen EUR/SEK)', ...
  'CC_transl_daily [SEK] (pure EUR/SEK at frozen foreign)', ...
  'CC_cross_daily [SEK] (Δfor x ΔEUR/SEK)', ...
  'CC_total_LYRates_daily [SEK]', ...
  'CC_trans_LYRates_daily [SEK]', ...
  'CC_transl_LYRates_daily [SEK]', ...
  'TI_Bonds_cumulative [SEK]', ...
  'TI_BondsBOM_cumulative [SEK]', ...
  'OCI_cumulative [SEK]', ...
  'CC_total_cumulative [SEK]', ...
  'CC_trans_cumulative [SEK]', ...
  'CC_transl_cumulative [SEK]', ...
  'CC_cross_cumulative [SEK]', ...
  'CC_total_LYRates_cumulative [SEK]', ...
  'CC_trans_LYRates_cumulative [SEK]', ...
  'CC_transl_LYRates_cumulative [SEK]'};
fxData = [dateStrs, num2cell([ ...
  dr.dFX_trans, dr.dFX_trans_BOM, dr.dFX_transl, dr.dFX_cc_total, ...
  dr.dFX_cc_trans, dr.dFX_cc_transl, dr.dFX_cc_cross, ...
  dr.dFX_cc_LY_total, dr.dFX_trans_CC_LY, dr.dFX_transl_CC_LY, ...
  cumsum(dr.dFX_trans), cumsum(dr.dFX_trans_BOM), cumsum(dr.dFX_transl), cumsum(dr.dFX_cc_total), ...
  cumsum(dr.dFX_cc_trans), cumsum(dr.dFX_cc_transl), cumsum(dr.dFX_cc_cross), ...
...
% -------------------------------------------------------------------------
% Sheet 18: Quarterly_OCI — Translation (OCI) all-methods quarterly comparison
%   PAM_OCI (Eq.4.46), M1 daily-spot OCI, M2 weekly/monthly/quarterly OCI,
%   cumulative columns and per-quarter diffs versus PAM.
% -------------------------------------------------------------------------
hdrOCI = {'Quarter', ...
  'PAM_OCI [SEK] (Eq.4.46)', ...
  'Method1_OCI [SEK]', ...
  'Method2_Weekly_OCI [SEK]', ...
  'Method2_Monthly_OCI [SEK]', ...
  'Method2_Quarterly_OCI [SEK]', ...
  'Cumul_PAM_OCI [SEK]', ...
  'Cumul_Method1_OCI [SEK]', ...
  'Cumul_Method2_Weekly_OCI [SEK]', ...
  'Cumul_Method2_Monthly_OCI [SEK]', ...
  'Cumul_Method2_Quarterly_OCI [SEK]', ...
  'Diff_M1_vs_PAM [SEK]', ...
  'Diff_M2w_vs_PAM [SEK]', ...
  'Diff_M2m_vs_PAM [SEK]', ...
  'Diff_M2q_vs_PAM [SEK]'};
ociData = [qLabels, num2cell([ ...
  OCI_q, m1OCI_q, m2OCI_w_q, m2OCI_m_q, m2OCI_qt_q, ...
  cumsum(OCI_q), cumsum(m1OCI_q), cumsum(m2OCI_w_q), cumsum(m2OCI_m_q), cumsum(m2OCI_qt_q), ...
  m1OCI_q - OCI_q, m2OCI_w_q - OCI_q, m2OCI_m_q - OCI_q, m2OCI_qt_q - OCI_q])];
ociTotRow = [{'TOTAL'}, num2cell([ ...
  sum(OCI_q), sum(m1OCI_q), sum(m2OCI_w_q), sum(m2OCI_m_q), sum(m2OCI_qt_q), ...
  sum(OCI_q), sum(m1OCI_q), sum(m2OCI_w_q), sum(m2OCI_m_q), sum(m2OCI_qt_q), ...
  sum(m1OCI_q)-sum(OCI_q), sum(m2OCI_w_q)-sum(OCI_q), ...
  sum(m2OCI_m_q)-sum(OCI_q), sum(m2OCI_qt_q)-sum(OCI_q)])];
writecell([hdrOCI; ociData; ociTotRow], xlFile, 'Sheet', 'Quarterly_OCI');
fprintf('  Sheet 18/19: Quarterly_OCI\n');

% -------------------------------------------------------------------------
% Sheet 19: Annual_OCI_Comparison — annual totals across all methods.
%   One row per calendar year with cumulative growth, plus a TOTAL row.
% -------------------------------------------------------------------------
hdrOCIa = {'Year', ...
  'PAM_OCI [SEK] (Eq.4.46)', ...
  'Method1_OCI [SEK]', ...
  ...
  'Cumul_Method2_Monthly_OCI [SEK]', ...
  'Cumul_Method2_Quarterly_OCI [SEK]', ...
  'Diff_M1_vs_PAM_annual [SEK]', ...
  'Diff_M2w_vs_PAM_annual [SEK]', ...
  'Diff_M2m_vs_PAM_annual [SEK]', ...
  'Diff_M2q_vs_PAM_annual [SEK]'};
yLabels = num2cell(uniqueYears(:));
ociaData = [yLabels, num2cell([ ...
  OCI_a, m1OCI_a, m2OCI_w_a, m2OCI_m_a, m2OCI_qt_a, ...
  cumsum(OCI_a), cumsum(m1OCI_a), cumsum(m2OCI_w_a), cumsum(m2OCI_m_a), cumsum(m2OCI_qt_a), ...
  m1OCI_a - OCI_a, m2OCI_w_a - OCI_a, m2OCI_m_a - OCI_a, m2OCI_qt_a - OCI_a])];
ociaTotRow = [{'TOTAL'}, num2cell([ ...
  sum(OCI_a), sum(m1OCI_a), sum(m2OCI_w_a), sum(m2OCI_m_a), sum(m2OCI_qt_a), ...
  sum(OCI_a), sum(m1OCI_a), sum(m2OCI_w_a), sum(m2OCI_m_a), sum(m2OCI_qt_a), ...
  sum(m1OCI_a)-sum(OCI_a), sum(m2OCI_w_a)-sum(OCI_a), ...
  sum(m2OCI_m_a)-sum(OCI_a), sum(m2OCI_qt_a)-sum(OCI_a)])];
writecell([hdrOCIa; ociaData; ociaTotRow], xlFile, 'Sheet', 'Annual_OCI_Comparison');
fprintf('  Sheet 19/21: Annual_OCI_Comparison\n');

% -------------------------------------------------------------------------
% Sheet 20: Quarterly_CC — Constant-currency all-methods quarterly comparison
%   PAM CC three-way (trans + transl + cross + total) vs M1, M2avg, M2close totals.
%   Industry CC sourced from m1.cc (= m2.cc; identical computation).
% -------------------------------------------------------------------------
% Align industry CC quarterlies to the PAM quarter grid
ccM1_q  = zeros(nPeriods,1); ccAvg_q = zeros(nPeriods,1); ccCls_q = zeros(nPeriods,1);
if isfield(m1, 'cc')
  for p = 1:nPeriods
    [~, mp] = min(abs(m1.cc.periodEndDates - periodDates(p+1)));
    if abs(m1.cc.periodEndDates(mp) - periodDates(p+1)) <= 5
      ccM1_q(p)  = m1.cc.M1.quarterly_TI(mp)    + m1.cc.M1.quarterly_OCI(mp);
      ccAvg_q(p) = m1.cc.avg.quarterly_TI(mp)   + m1.cc.avg.quarterly_OCI(mp);
      ccCls_q(p) = m1.cc.close.quarterly_TI(mp) + m1.cc.close.quarterly_OCI(mp);
    end
  end
end

hdrCC = {'Quarter', ...
  'PAM_CC_total [SEK] (Eq.4.47)', 'PAM_CC_trans [SEK]', 'PAM_CC_transl [SEK]', 'PAM_CC_cross [SEK]', ...
  'Method1_CC_total [SEK]', 'Method2avg_CC_total [SEK]', 'Method2close_CC_total [SEK]', ...
  'Cumul_PAM_CC_total [SEK]', 'Cumul_M1_CC_total [SEK]', 'Cumul_M2avg_CC_total [SEK]', 'Cumul_M2close_CC_total [SEK]', ...
  'Diff_M1_vs_PAM [SEK]', 'Diff_M2avg_vs_PAM [SEK]', 'Diff_M2close_vs_PAM [SEK]'};
ccData = [qLabels, num2cell([ ...
  CCt_q, CCtr_q, CCtl_q, CCx_q, ...
  ccM1_q, ccAvg_q, ccCls_q, ...
  cumsum(CCt_q), cumsum(ccM1_q), cumsum(ccAvg_q), cumsum(ccCls_q), ...
  ccM1_q - CCt_q, ccAvg_q - CCt_q, ccCls_q - CCt_q])];
ccTotRow = [{'TOTAL'}, num2cell([ ...
  sum(CCt_q), sum(CCtr_q), sum(CCtl_q), sum(CCx_q), ...
  sum(ccM1_q), sum(ccAvg_q), sum(ccCls_q), ...
  sum(CCt_q), sum(ccM1_q), sum(ccAvg_q), sum(ccCls_q), ...
  sum(ccM1_q)-sum(CCt_q), sum(ccAvg_q)-sum(CCt_q), sum(ccCls_q)-sum(CCt_q)])];
writecell([hdrCC; ccData; ccTotRow], xlFile, 'Sheet', 'Quarterly_CC');
fprintf('  Sheet 20/21: Quarterly_CC\n');

% -------------------------------------------------------------------------
% Sheet 21: Annual_CC_Comparison — annual totals across all CC methods.
% -------------------------------------------------------------------------
ccM1_a = zeros(nYears,1); ccAvg_a = zeros(nYears,1); ccCls_a = zeros(nYears,1);
for y = 1:nYears
  qM = (qYears == uniqueYears(y));
  ccM1_a(y)  = sum(ccM1_q(qM));
  ccAvg_a(y) = sum(ccAvg_q(qM));
  ccCls_a(y) = sum(ccCls_q(qM));
end
...
hdrCCa = {'Year', ...
  ...
  'Cumul_PAM_CC_total [SEK]', 'Cumul_M1_CC_total [SEK]', 'Cumul_M2avg_CC_total [SEK]', 'Cumul_M2close_CC_total [SEK]', ...
  'Diff_M1_vs_PAM [SEK]', 'Diff_M2avg_vs_PAM [SEK]', 'Diff_M2close_vs_PAM [SEK]'};
ccaData = [num2cell(uniqueYears), num2cell([ ...
  CCt_a, CCtr_a, CCtl_a, CCx_a, ...
  ccM1_a, ccAvg_a, ccCls_a, ...
  cumsum(CCt_a), cumsum(ccM1_a), cumsum(ccAvg_a), cumsum(ccCls_a), ...
  ccM1_a - CCt_a, ccAvg_a - CCt_a, ccCls_a - CCt_a])];
ccaTotRow = [{'TOTAL'}, num2cell([ ...
  sum(CCt_a), sum(CCtr_a), sum(CCtl_a), sum(CCx_a), ...
  sum(ccM1_a), sum(ccAvg_a), sum(ccCls_a), ...
  sum(CCt_a), sum(ccM1_a), sum(ccAvg_a), sum(ccCls_a), ...
  sum(ccM1_a)-sum(CCt_a), sum(ccAvg_a)-sum(CCt_a), sum(ccCls_a)-sum(CCt_a)])];
writecell([hdrCCa; ccaData; ccaTotRow], xlFile, 'Sheet', 'Annual_CC_Comparison');
fprintf('  Sheet 21/21: Annual_CC_Comparison\n');

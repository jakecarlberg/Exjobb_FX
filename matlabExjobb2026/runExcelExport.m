...
aData = [num2cell(uniqueYears), num2cell([...
  TI_a, TI_BOM_a, OCI_a, CCt_a, CCtr_a, CCtl_a, CCt_LY_a, CCtr_LY_a, CCtl_LY_a, ...
  m2TI_w_a, m2OCI_w_a, m2TI_m_a, m2OCI_m_a, m2TI_qt_a, m2OCI_qt_a, ...
  TI_a-m2TI_w_a,     TI_BOM_a-m2TI_w_a,  OCI_a-m2OCI_w_a, ...
  TI_a-m2TI_m_a,     TI_BOM_a-m2TI_m_a,  OCI_a-m2OCI_m_a, ...
  TI_a-m2TI_qt_a,    TI_BOM_a-m2TI_qt_a, OCI_a-m2OCI_qt_a])];
totRow = [{'TOTAL'}, num2cell([...
  sum(TI_a), sum(TI_BOM_a), sum(OCI_a), sum(CCt_a), sum(CCtr_a), sum(CCtl_a), ...
  sum(CCt_LY_a), sum(CCtr_LY_a), sum(CCtl_LY_a), ...
  sum(m2TI_w_a), sum(m2OCI_w_a), sum(m2TI_m_a), sum(m2OCI_m_a), sum(m2TI_qt_a), sum(m2OCI_qt_a), ...
  sum(TI_a)-sum(m2TI_w_a),  sum(TI_BOM_a)-sum(m2TI_w_a),  sum(OCI_a)-sum(m2OCI_w_a), ...
  sum(TI_a)-sum(m2TI_m_a),  sum(TI_BOM_a)-sum(m2TI_m_a),  sum(OCI_a)-sum(m2OCI_m_a), ...
  sum(TI_a)-sum(m2TI_qt_a), sum(TI_BOM_a)-sum(m2TI_qt_a), sum(OCI_a)-sum(m2OCI_qt_a)])];
...
hdrPAM = {'Date', ...
  ...
  'dFX_trans_EUR [EUR] (Trans FX in EUR — Bonds only = dPf+dPc, Eq.4.45)', ...
  'dFX_trans_BOM_EUR [EUR] (Trans FX in EUR — Bonds+BOM = dPf+dPxi_FX+dPc)'};

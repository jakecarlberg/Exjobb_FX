...
ii = find(contains(tmp(ind), 'CKD'));
for i=1:length(ii)
  tmp(ind(ii(i))) = replace(tmp(ind(ii(i))), 'CKD', '');
end
ii = find(contains(tmp(ind), 'KIT'));
for i=1:length(ii)
  tmp(ind(ii(i))) = replace(tmp(ind(ii(i))), 'KIT', '');
end
ii = find(contains(tmp(ind), '-'));
for i=1:length(ii)
  tmp(ind(ii(i))) = replace(tmp(ind(ii(i))), '-', '');
end
ii = find(contains(tmp(ind), 'T'));
for i=1:length(ii)
  tmp(ind(ii(i))) = replace(tmp(ind(ii(i))), 'T', '');
end
itemNumbers(ind)  = str2double(tmp(ind));
